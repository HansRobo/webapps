...
以下のフィールドは任意。情報が判明している製品のみ追加する。存在する場合は通常の `{ value, unit, refs, note? }` 形式で記述。
...
| フィールド            | 説明                                      | 単位例          |
|-----------------------|-------------------------------------------|----------------|
| `operatingTemperature`| 動作温度範囲（例: "-40 ~ 85 °C"）          | `null`（文字列）|
| `shockVibration`      | 耐衝撃・耐振動規格                         | `null`（文字列）|

**正規化補助:**

| フィールド   | 説明                                      | 単位例          |
|-------------|-------------------------------------------|----------------|
| `powerMax`  | 最大消費電力（ヒーター・起動時含む）         | `"W"`           |
| `precision` | ばらつき/再現性（Precision/Repeatability）   | `null`（文字列）|

---

## release フィールドの書き方（比較可能パラメータ）

`release` は製品一覧ソート、詳細フィルタ、パラメータ解説・分布（`#/parameters/releaseYear`）などで「比較可能なパラメータ」として扱われます。
...
// ❌ NG: 年情報が含まれていないとバリデーションで警告が出る
release: { value: "現行販売", refs: [1] }
release: { value: "旧世代・販売終了", refs: [1] }
```

- 必ず正規表現 `(19\d\d|20\d\d)` で西暦年が抽出できる表記にしてください。
- 補足説明や発表イベント（CES、上海モーターショーなど）は括弧書き等で併記可能です。
